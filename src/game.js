  ...
  async evaluate() {
    return new Promise(resolve => {
      this.game.renderer.snapshot(async (image) => {
        var prediction;
        this.evalulationTraining++;
        tf.engine().startScope();
        var result = await this.model.predict(this.shapeImage(image));
        prediction = result.tanh().dataSync();
        console.log(result.dataSync(), prediction)

        /*
        var input = this.shapeImage(image);

        const trainResult = await this.model.fit(input, result, {
          epochs: 1,
          batchSize: this.batchSize,
          yieldEvery: 'epoch'
        });

        console.log(this.evalulationTraining, trainResult);
        */
